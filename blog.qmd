...
-   **doc_id**: A unique identifier for each course, constructed by
    ...
    For example, 'BC-THE-2300' refers to a "Survey Of Dramatic Literature" course in the Theatre Arts department at Broward College.
...
-   **text**: The course description text that was scraped from
    ...
    text that we analyze using text mining techniques. 
...
`corpus()` function in `quanteda.` 
```{r}
corpus <- corpus(courses)
```

The output of this code is a corpus object that consists of the course
descriptions from our data frame. The corpus contains metadata about the documents, each of which correspond to a course description. For instance, the first few documents in the corpus look something like this:
...
Each line represents a document in the corpus, with the document ID followed by a snippet of the course description. This corpus will
serve as the basis for our subsequent text analysis.

Next, we extract the tokens in the
...
While our methodology provides valuable insights, it's crucial to acknowledge its limitations. For instance, our current dataset doesn't capture the quality of WBL experiences or equity dimensions, such as whether the programs are paid - a significant factor given the work commitments of many community college students. Additionally, the scope of our analysis is limited by the dictionaries we compiled, the sample of schools considered, and the inherent constraints of course descriptions, which may not fully encapsulate all aspects of a course's content or its WBL opportunities. Therefore, future research could build on this work to develop more robust measures of WBL prevalence and to investigate the quality and equity of these opportunities.
